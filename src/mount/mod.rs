...
#[cfg(feature = "purerust")]
mod fuse_pure;
pub mod mount_options;

#[cfg(any(test, feature = "libfuse"))]
use fuse2_sys::fuse_args;
#[cfg(any(test, feature = "purerust", feature = "libfuse2"))]
use std::io;
...
fn with_fuse_args<T, F: FnOnce(&fuse_args) -> T>(options: &[MountOption], f: F) -> T {
    ...
    for x in options {
        args.extend_from_slice(&[CString::new("-o").unwrap(), CString::new(option_to_string(x)).unwrap()]);
    }
...
mod test {
    ...
    fn fuse_args() {
        with_fuse_args(
            &[MountOption::CUSTOM("foo".into()), MountOption::CUSTOM("bar".into())],
            |args| {
                let v: Vec<_> = (0..args.argc)
                    .map(|n| unsafe { CStr::from_ptr(*args.argv.offset(n as isize)).to_str().unwrap() })
                    .collect();
